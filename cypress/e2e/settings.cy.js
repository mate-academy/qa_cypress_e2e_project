...
describe('Settings page', () => {
  ...
  it('should provide an ability to update username', () => {
    cy.visit('/#/settings');
    cy.getByDataQa('username').clear().type(userSettings.username);
    cy.intercept('POST', '**/user').as('updateUser');
    cy.getByDataQa('updateButton').click();
  ...
  it('should provide an ability to update bio', () => {
    cy.visit('/#/settings');
    cy.getByDataQa('BioInfo').clear().type(userSettings.bio);
    cy.intercept('POST', '**/user**').as('updateUser');
    cy.getByDataQa('updateButton').click();
    cy.wait('@updateUser').then((interception) => {
  ...
  it('should provide an ability to update password', () => {
    cy.visit('/#/settings');
    cy.getByDataQa('Password').clear().type(userSettings.password);
    cy.intercept('POST', '**/user').as('updateUser');
    cy.getByDataQa('updateButton').click();
    ...
    cy.visit('/#/settings');
    cy.reload();
    cy.getByDataQa('username-link').should('contain.text', user.username);
  });
});